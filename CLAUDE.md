...
- Projects: `/projects` (in the main nav) shows the owner's install videos from `PROJECT_VIDEOS` in `src/lib/projects-data.ts`, framed as a small sample of the total (`PROJECTS_COMPLETED`, currently 125, the owner's count, used by the hero stat strip and the counters too). Videos in `public/videos/projects/` are 720p re-encodes with the PowerNexa logo burned in, faint (about 28% opacity) and centred, plus a poster JPG. Make new ones the same way (ffmpeg overlay of an alpha-faded crop of `logo-transparent.png` at 380px wide, `-crf 28 -maxrate 850k`, `+faststart`). Keep the source phone videos out of git. Labels use only the owner's own captions; Mowe-Ibafo came with a location only, so its label names visible parts without sizes. The homepage `ProofSection` (right after TrustedBy) has 4 count-up counters (`CountUp`, counts from 1 on scroll, reduced-motion safe) next to the Elegushi video. Every counter figure must stay provable (owner's count, the Elegushi install's specs, `LAGOS_AREAS.length`).
