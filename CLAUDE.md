...
- The brand logo source (`public/images/logo.png`) is a flat RGB PNG with a white background baked in, no alpha channel. The header/admin pages use it directly since their background is white. The footer is navy, so it uses `public/images/logo-transparent.png` (the same artwork with the white background alpha-matted out via a one-off `sharp` script) inside a small white rounded chip, not the old `brightness-0 invert` filter — that filter turned the whole non-transparent bounding box solid white, which made the footer logo invisible. If the source logo is ever replaced, regenerate `logo-transparent.png` the same way (or export a proper transparent PNG from the design file) before wiring it into the footer.
